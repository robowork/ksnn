...
ap.add_argument('-i', '--image_dir', required=True,
                help='path to directory conatining the images')
...
# get filename of all the images in the directory
all_imgs = os.listdir(args.image_dir)
all_imgs.sort()
...
for i in all_imgs[:5]:
    image_path = os.path.join(args.image_dir, i)
    # read input image
    ...
    # cv2.waitKey()
    output_path = os.path.join(output_dir_name, "output_"+i)
